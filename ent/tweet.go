...
type Tweet struct {
	...
	// Tweet holds the value of the "tweet" field.
...
func (*Tweet) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case tweet.FieldID, tweet.FieldTweet, tweet.FieldUserID:
...
func (t *Tweet) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case tweet.FieldTweet:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field tweet", values[i])
			} else if value.Valid {
				t.Tweet = value.String
			}
		case tweet.FieldCreatedAt:
